...
INTERSTRING_16 = [-32768 & 0xffff,-129 & 0xffff,28 & 0xffff,255 & 0xffff,256 & 0xffff,512 & 0xffff,1000 & 0xffff,1024 & 0xffff,4096 & 0xffff,32767 & 0xffff]
...
# cross over
def cross_over_mutate(buf, add_buf):    # Splice two parts from two different tes cases to form a new test case
	buf_len = len(buf)
	if(buf_len < 4):
		buf[0:0] = add_buf
		return buf, len(buf) - buf_len
	index = UR(buf_len - 1)
	buf[index:index] = add_buf
	fix_len = len(buf) - buf_len
	return buf, fix_len
